 ...
 * @since        3.1.2
 */
class MagicgalleryObserverEntity extends JTableObserver
{
    /**
     ...
     * @return  MagicgalleryObserverEntity
     *
     * @since   3.1.2
     */
    ...
    public function onBeforeDelete($pk)
    {
        if ((int)$this->table->get('id') > 0) {
            ...
            $gallery->load($this->table->get('gallery_id'));
            ...
            if ($mediaFolder) {
                if ($this->table->get('image')) {
                    $image = JPath::clean($mediaFolder . '/'. $this->table->get('image'), '/');

                    if ($storageFilesystem->has($image)) {
                        $storageFilesystem->delete($image);
                    }
                }

                if ($this->table->get('thumbnail')) {
                    $thumbnail = JPath::clean($mediaFolder . '/' . $this->table->get('thumbnail'), '/');

                    if ($storageFilesystem->has($thumbnail)) {
                        $storageFilesystem->delete($thumbnail);
                    }
                }
            }
        }
